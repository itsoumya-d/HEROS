...
- **`evolve/spec/skill_score.0`** — The one pure-compute kernel (confidence scoring) specified for Zero (fixed-point Wilson lower bound). Spec-only until a Zero compiler is in CI; the awk reference in the bridge is the tested source of truth, with a documented parity contract (`evolve/spec/README.md`).
